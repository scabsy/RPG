...
local function isOdd( number )
    if number % 2 == 1 then return true end
    return false
end
math.isOdd = isOdd
...
local function isEven( number )
    if number % 2 == 0 then return true end
    return false
end
math.isEven = isEven
...
math.sumDigits = sumDigits


-- checks if a number is divisible by three
local function isDivisibleBy3( number )
    local sum = sumDigits( number )
    if sum % 3 == 0 then return true end
    return false
end
math.isDivisibleBy3 = isDivisibleBy3



-- checks if a number is divisible by four
local function isDivisibleBy4( number )

    -- get rid of negative number
    local number = math.abs( number )

    -- if number has at least two digits
    if number >= 10 then
        -- convert to string
        local strNumber = tostring( number )

        -- get the last two digits
        local lastTwo = tonumber( strNumber:sub(-2) )

        if lastTwo % 4 == 0 then return true end
    else
        if number % 4 == 0 then return true end
    end

    return false
end
math.isDivisibleBy4 = isDivisibleBy4
...
local function randomOne()
    local randomVal = math.round( math.random() )
...
--[[
    ...
    Params:
        ...
        lineB - The second line { a={x,y}, b={x,y} }
]]--
...
local function forcesByAngle(totalForce, angle)
    ...
    forces.x = -math.sin(radians) * totalForce
    forces.y = -math.cos(radians) * totalForce
...
-- converts an angle to be a positive value between 0 and 360
